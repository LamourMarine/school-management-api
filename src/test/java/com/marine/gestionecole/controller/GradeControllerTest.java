...
public class GradeControllerTest {
    ...
    void shouldCreateGrade() throws Exception {
        ...
        mockMvc
        .perform(post("/api/grades").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(g1)))
    ...
    void shouldUpdateGrade() throws Exception {
        ...
        mockMvc
        .perform(put("/api/grades/{id}", 1L).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(g1)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.student.firstName").value("Alice"))
        .andExpect(jsonPath("$.student.lastName").value("Dupont"))
        .andExpect(jsonPath("$.score").value(13.0))
